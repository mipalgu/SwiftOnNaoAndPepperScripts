...
swift_include_flags=`echo "$INCLUDE_FLAGS -I$INSTALL_PREFIX/lib/swift -I$INSTALL_PREFIX/lib/swift/clang/include" | sed 's/ /;/g' | sed 's/;/ /g'`
...
then
    ...
    PATH="$CROSS_DIR/bin:$PATH" CC="$HOST_CLANG" CXX="$HOST_CLANGXX" CPATH="$CPATH" LIBRARY_PATH="$LIBRARY_PATH" LD="$CROSS_DIR/bin/$TRIPLE-ld.gold" cmake -G "Ninja" \
	    ...
	    -DCMAKE_Swift_FLAGS="-I$INSTALL_PREFIX/lib/swift/linux/i686 $swift_include_flags -I$LFS/usr/include -I$LFS/include -I$CROSS_DIR/lib/gcc/$TRIPLE/$GCC_VERSION/include-fixed -sdk $LFS $swift_link_flags -target $TRIPLE" \
	    -DAST_TARGET="$TRIPLE" \
