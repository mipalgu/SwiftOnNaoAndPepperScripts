...
if [ ! -f $LLBUILD_BUILD_DIR/.llbuild-build-cross ]
then
    ...
    PATH="$CROSS_DIR/bin:$PATH" CC="$HOST_CLANG" CXX="$HOST_CLANGXX" CPATH="$CPATH" LIBRARY_PATH="$LIBRARY_PATH" LD="$CROSS_DIR/bin/$TRIPLE-ld.gold" cmake -G "Ninja" \
	    ...
	    -DCMAKE_CXX_COMPILER_TARGET="$TRIPLE" \
            ...
            -DCMAKE_EXE_LINKER_FLAGS="-gcc-toolchain $CROSS_DIR $LINK_FLAGS -latomic" \
            -DCMAKE_SHARED_LINKER_FLAGS="-gcc-toolchain $CROSS_DIR $LINK_FLAGS -latomic" \
    ...
    cd $SRC_DIR
    PATH="$CROSS_DIR/bin:$PATH" cmake --build $LLBUILD_BUILD_DIR
    PATH="$CROSS_DIR/bin:$PATH" cd $LLBUILD_BUILD_DIR && ninja install
    cd $WD
    /usr/local/var/swiftenv/bin/swiftenv local $swiftenv_swift_version
    touch $LLBUILD_BUILD_DIR/.llbuild-build-cross
fi
