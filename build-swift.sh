...
function swift() {
    ...
    PATH="$LFS/bin:$PATH" CC="$HOST_CLANG" CXX="$HOST_CLANGXX" CPATH="$CPATH" LIBRARY_PATH="$LIBRARY_PATH" cmake -G "Ninja" \
      ...
      -DSWIFT_PATH_TO_CMARK_SOURCE="$SRC_DIR/apple/cmark" \
      -DSWIFT_PATH_TO_CMARK_BUILD="$CMARK_BUILD_DIR" \
      -DSWIFT_CMARK_LIBRARY_DIR="$CMARK_BUILD_DIR/src" \
      -DSWIFT_PATH_TO_LLVM_SOURCE="$SRC_DIR/apple/llvm" \
      -DSWIFT_PATH_TO_LLVM_BUILD="$LLVM_BUILD_DIR" \
      -DSWIFT_PATH_TO_CLANG_SOURCE="$SRC_DIR/apple/clang" \
      ...
      -DSWIFT_SOURCE_DIR="$SRC_DIR/apple/swift" \
      ...
      -DLIBXML2_INCLUDE_DIR="$LFS/include/libxml2" \
      -DLIBXML2_LIBRARY="$LFS/lib/libxml2.so" \
